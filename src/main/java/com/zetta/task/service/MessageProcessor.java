...
public class MessageProcessor {
	...
	public MessageProcessor(ConditionEngine conditionEngine, TransformationEngine transformationEngine,
			MessageStateRepository messageStateRepository, StateDeltaRepository stateDeltaRepository,
			MessageProducer messageProducer, ObjectMapper objectMapper, ResourceLoader resourceLoader) {
	...
	private MessageState persistState(String messageId, JsonNode data) throws IOException {
		...
		MessageState state = messageStateRepository.findByMessageId(messageId).orElseGet(() -> {
	...
	private String calculateChanges(String beforeState, String afterState) throws IOException {
		...
		// In a more sophisticated implementation, this would calculate field-by-field
		// differences
